...
export function formatNewFollowAlert(
  ...
  const location = newFollow.location ? `\n📍 ${newFollow.location}` : '';
  ...
  return (
    ...
    `━━━━━━━━━━━━━━━━━━\n` +
    `🔗 [View Profile](https://twitter.com/${newFollow.username}) \\| [Search CA](https://dexscreener.com/search?q=${newFollow.username})`
  );
}
...
export function formatConvergenceAlert(data: ConvergenceAlertData): string {
  const categoryTags = data.categories.length > 0
...
export function formatDailyDigest(
  ...
  for (const [category, catEntries] of categorizedEntries) {
    ...
    lines.push(`*${escapeMarkdown(category)}* \\(${catEntries.length} new follows\\)`);

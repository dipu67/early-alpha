...
import { sendTelegramAlert, topicForSlug } from "../tg/sendAlert.js";
...
async function handleTweet(ctx: ListSyncCtx, slug: string, tweet: TweetData): Promise<void> {
  ...
  const signals = detectSignals(tweet.text, slug);
  ...
  if (slug === ALPHA_SLUG && accountId) {
    const found = classifyText(tweet.text);
    if (found.length > 0) {
      ...
      if (result && result.added.length > 0) {
        ...
        await sendTelegramAlert(
          ...
          "MarkdownV2",
          topicForSlug(result.added[0] ?? slug),
        );
      }
    }
  }

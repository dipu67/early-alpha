...
const worker = new Worker(
  ...
  async (job) => {
    if (job.name === "reconcile-lists") {
      ...
      console.log(
        `[list-worker] poll-watching checked=${r.checked} timelines=${r.timelines} ` +
          `alerted=${r.alerted} (signal=${r.alertedSignal} row=${r.alertedRow}) ` +
          `seeded=${r.seeded} skippedDisabled=${r.skippedDisabled} held=${r.held} rateLimited=${r.rateLimited}`,
      );
